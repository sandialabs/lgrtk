...
convert_tet_mesh_to_meshless(const input& in, state& st)
{
  auto const num_points =
      st.elements.size() * in.otm_material_points_to_add_per_element;
  ...
  auto const points_in_support =
      hpc::make_counting_range(in.otm_material_points_to_add_per_element);
  auto const elements_to_points = st.elements * points_in_support;

  hpc::device_vector<int, point_index> nodes_in_support_counts(
      st.points.size(), st.nodes_in_element.size());
  ...
  if (in.xp_transform) {
    in.xp_transform(
        st.points,
        st.points_to_point_nodes,
        st.point_nodes_to_nodes,
        st.x,
        st.xp);
  }
...
otm_update_h(state& st)
{
  ...
  auto       h_min_func             = [=] HPC_DEVICE(point_index const point) {
    ...
    for (auto n : nodes_in_support[point]) {
      ...
      auto node_pt_dist =
          hpc::norm(nodes_to_x[node].load() - mat_pts_to_x[point].load());
