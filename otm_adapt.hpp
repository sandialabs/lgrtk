...
void
otm_populate_new_nodes_linear(
    state& s,
    hpc::device_array_vector<NodeIndexArray, node_index> const&
        interpolate_from_nodes)
{
  ...
  hpc::for_each(
      hpc::device_policy(), interpolate_from_nodes, maxent_interpolator);
}

template <typename PointIndexArray>
void
otm_populate_new_points_linear(
    state& s,
    hpc::device_array_vector<PointIndexArray, point_index> const&
        interpolate_from_points)
{
  ...
  auto       maxent_interpolator     = [=] HPC_DEVICE(point_index const point) {
    ...
    for (auto&& source_point : source_range) {
      ...
      auto const rotation_vector_plastic =
          hpc::rotation_vector_from_rotation_tensor(Rp);
  ...
  hpc::for_each(
      hpc::device_policy(), interpolate_from_points, maxent_interpolator);
}
...
struct otm_adapt_state
{
  ...
  hpc::device_array_vector<hpc::array<node_index, 2, int>, node_index>
      interpolate_from_nodes;
  hpc::device_array_vector<hpc::array<point_index, 2, int>, point_index>
                                   interpolate_from_points;
