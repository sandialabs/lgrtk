...
inline HPC_HOST_DEVICE hpc::adimensional<double>
                       triangle_quality(hpc::array<hpc::position<double>, 3> const x) noexcept
{
...
HPC_NOINLINE inline void
update_triangle_quality(state& s) noexcept
{
...
/* Per:
   ...
   As such, our "quality" is the inverse of this quality measure to the fourth
   power
  */
HPC_NOINLINE inline void
update_tetrahedron_quality(state& s)
{
  auto const points_to_V           = s.V.cbegin();
  auto const point_nodes_to_grad_N = s.grad_N.cbegin();
  auto const elements_to_quality   = s.quality.begin();
  ...
  auto       functor = [=] HPC_DEVICE(element_index const element) {
    ...
    elements_to_quality[element] =
        (V * V) * (sum_g_i_sq * sum_g_i_sq * sum_g_i_sq);
  };
  hpc::for_each(hpc::device_policy(), s.elements, functor);
}

void
update_quality(input const& in, state& s)
{
  switch (in.element) {
...
struct adapt_state
{
  ...
  hpc::device_array_vector<hpc::array<node_index, 2, int>, node_index>
                                     interpolate_from;
...
struct eval_cavity
{
  ...
  hpc::array<hpc::array<int, nodes_per_element>, max_shell_elements>
                                                 shell_elements_to_shell_nodes;
  hpc::array<material_index, max_shell_elements> shell_elements_to_materials;
  hpc::array<hpc::adimensional<double>, max_shell_elements>
                                      shell_element_qualities;
    ...
    int& best_swap_edge_node)
{
  ...
  for (int element = 0; element < c.num_shell_elements; ++element) {
    ...
    if (c.shell_elements_to_shell_nodes[element][(node_in_element + 1) % 3] ==
        edge_node) {
      loop_elements[1] = element;
      loop_nodes[1] =
          c.shell_elements_to_shell_nodes[element][(node_in_element + 2) % 3];
    }
    if (c.shell_elements_to_shell_nodes[element][(node_in_element + 2) % 3] ==
        edge_node) {
      loop_elements[0] = element;
      loop_nodes[0] =
          c.shell_elements_to_shell_nodes[element][(node_in_element + 1) % 3];
    }
  }
  if (loop_elements[0] == -1 || loop_elements[1] == -1) { return; }
  if (c.shell_elements_to_materials[loop_elements[0]] !=
      c.shell_elements_to_materials[loop_elements[1]]) {
    return;
  }
    ...
    int& best_split_edge_node)
{
  ...
  for (int element = 0; element < c.num_shell_elements; ++element) {
    ...
    int const                            center_node_in_element =
        c.shell_elements_to_node_in_element[element];
    int edge_node_in_element = -1;
    for (int node_in_element = 0; node_in_element < 3; ++node_in_element) {
      int const shell_node =
          c.shell_elements_to_shell_nodes[element][node_in_element];
...
{
  if (!c.shell_nodes_to_materials[edge_node].contains(
          c.shell_nodes_to_materials[center_node]))
    return;
  constexpr double min_acceptable_quality = 0.2;
  auto const       h1                     = c.shell_nodes_to_h[center_node];
  auto const       h2                     = c.shell_nodes_to_h[edge_node];
  ...
  for (int element = 0; element < c.num_shell_elements; ++element) {
    ...
    for (int node_in_element = 0; node_in_element < 3; ++node_in_element) {
      ...
      if (shell_node == edge_node) edge_node_in_element = node_in_element;
      material_index const element_material =
          c.shell_elements_to_materials[element];
  ...
  if ((center_materials - boundary_materials) == edge_materials &&
      target_materials.contains(center_materials)) {
    shortest_length         = lm;
...
HPC_NOINLINE inline void
evaluate_triangle_adapt(input const& in, state const& s, adapt_state& a)
{
  auto const nodes_to_node_elements    = s.nodes_to_node_elements.cbegin();
  auto const node_elements_to_elements = s.node_elements_to_elements.cbegin();
  auto const node_elements_to_node_in_element =
  ...
  auto const boundary_materials =
      material_set::all(in.materials.size() + in.boundaries.size()) -
      material_set::all(in.materials.size());
  auto functor = [=] HPC_DEVICE(node_index const node) {
    ...
    for (auto const node_element : nodes_to_node_elements[node]) {
      ...
      for (auto const node_in_element : nodes_in_element) {
        ...
        int const                shell_node =
            find_or_append(c.num_shell_nodes, c.shell_nodes, node2);
        ...
        c.shell_elements_to_shell_nodes[shell_element]
      ...
      node_in_element_index const node_in_element =
          node_elements_to_node_in_element[node_element];
      c.shell_elements_to_node_in_element[shell_element] =
          hpc::weaken(node_in_element);
    }
    ...
    for (int edge_node = 0; edge_node < c.num_shell_nodes; ++edge_node) {
      ...
      evaluate_triangle_collapse(
          ...
          c,
          boundary_materials,
          shortest_collapse_edge,
          best_collapse_edge_node);
    }
...
HPC_NOINLINE inline void
choose_triangle_adapt(state const& s, adapt_state& a)
{
  auto const nodes_to_node_elements    = s.nodes_to_node_elements.cbegin();
  auto const node_elements_to_elements = s.node_elements_to_elements.cbegin();
  auto const elements_to_element_nodes = s.elements * s.nodes_in_element;
...
struct apply_cavity
{
  hpc::range_sum_iterator<node_element_index, node_index>
      nodes_to_node_elements;
  hpc::pointer_iterator<element_index const, node_element_index>
      node_elements_to_elements;
  hpc::pointer_iterator<node_in_element_index const, node_element_index>
      node_elements_to_nodes_in_element;
  hpc::
      counting_product<hpc::device_layout, element_index, node_in_element_index>
          elements_to_element_nodes;
  hpc::pointer_iterator<node_index const, element_node_index>
      old_element_nodes_to_nodes;
  hpc::pointer_iterator<element_index const, element_index>
      old_elements_to_new_elements;
  hpc::
      counting_product<hpc::device_layout, element_index, node_in_element_index>
                                                      new_elements_to_element_nodes;
  hpc::pointer_iterator<node_index const, node_index> old_nodes_to_new_nodes;
  hpc::pointer_iterator<node_index, element_node_index>
                                                      new_element_nodes_to_nodes;
  ...
  hpc::array_vector_iterator<
      hpc::array<node_index, 2, int>,
      hpc::device_layout,
      node_index>
      interpolate_from;
    ...
    node_index const   target_node)
{
  ...
  for (auto const node_element : c.nodes_to_node_elements[node]) {
    ...
    node_in_element_index const node_in_element =
        c.node_elements_to_nodes_in_element[node_element];
    node_in_element_index const plus1 =
        node_in_element_index((hpc::weaken(node_in_element) + 1) % 3);
    node_in_element_index const plus2 =
        node_in_element_index((hpc::weaken(node_in_element) + 2) % 3);
  ...
  element_index const new_element1 =
      c.old_elements_to_new_elements[loop_elements[0]];
  element_index const new_element2 =
      c.old_elements_to_new_elements[loop_elements[1]];
  using l_t              = node_in_element_index;
  auto new_element_nodes = c.new_elements_to_element_nodes[new_element1];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(0)]] =
      c.old_nodes_to_new_nodes[node];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(1)]] =
      c.old_nodes_to_new_nodes[loop_nodes[0]];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(2)]] =
      c.old_nodes_to_new_nodes[loop_nodes[1]];
  new_element_nodes = c.new_elements_to_element_nodes[new_element2];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(0)]] =
      c.old_nodes_to_new_nodes[target_node];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(1)]] =
      c.old_nodes_to_new_nodes[loop_nodes[1]];
  c.new_element_nodes_to_nodes[new_element_nodes[l_t(2)]] =
      c.old_nodes_to_new_nodes[loop_nodes[0]];
    ...
    node_index const   target_node)
{
  ...
  for (auto const node_element : c.nodes_to_node_elements[center_node]) {
    ...
    for (auto const node_in_element : c.nodes_in_element) {
      ...
      node_index const old_node =
          c.old_element_nodes_to_nodes[old_element_node];
    ...
    node_in_element_index const center_node_in_element =
        c.node_elements_to_nodes_in_element[node_element];
    element_index const new_element1 = c.old_elements_to_new_elements[element];
    auto const          new_element_nodes1 =
        c.new_elements_to_element_nodes[new_element1];
    new_nodes[center_node_in_element] = split_node;
    for (auto const node_in_element : c.nodes_in_element) {
      auto const new_element_node = new_element_nodes1[node_in_element];
      c.new_element_nodes_to_nodes[new_element_node] =
          new_nodes[node_in_element];
    }
    element_index const new_element2 = new_element1 + element_index(1);
    auto const          new_element_nodes2 =
        c.new_elements_to_element_nodes[new_element2];
...
HPC_NOINLINE inline void
apply_triangle_adapt(state const& s, adapt_state& a)
{
  apply_cavity c(s, a);
  ...
  auto       functor              = [=] HPC_DEVICE(node_index const node) {
    ...
    node_index const target_node = nodes_to_other_nodes[node];
    if (cavity_op::SWAP == op)
      apply_triangle_swap(c, node, target_node);
    else if (cavity_op::SPLIT == op)
      apply_triangle_split(c, node, target_node);
    else if (cavity_op::COLLAPSE == op)
      apply_triangle_collapse(c, node, target_node);
  };
  hpc::for_each(hpc::device_policy(), s.nodes, functor);
}

HPC_NOINLINE inline void
transfer_same_connectivity(state const& s, adapt_state& a)
{
  auto const new_elements_to_element_nodes =
      a.new_elements * s.nodes_in_element;
  auto const old_elements_to_element_nodes = s.elements * s.nodes_in_element;
  auto const new_elements_to_old_elements =
      a.new_elements_to_old_elements.cbegin();
  auto const nodes_in_element           = s.nodes_in_element;
  ...
  auto       functor = [=] HPC_DEVICE(element_index const new_element) {
    if (new_elements_are_same[new_element]) {
      ...
      element_index const old_element =
          new_elements_to_old_elements[new_element];
      auto const old_element_nodes = old_elements_to_element_nodes[old_element];
      for (auto const node_in_element : nodes_in_element) {
        auto const       new_element_node = new_element_nodes[node_in_element];
        auto const       old_element_node = old_element_nodes[node_in_element];
        node_index const old_node =
            node_index(old_element_nodes_to_nodes[old_element_node]);
...
HPC_NOINLINE inline void
transfer_element_materials(
    adapt_state&                                       a,
    hpc::device_vector<material_index, element_index>& data)
{
  hpc::device_vector<material_index, element_index> new_data(
      a.new_elements.size());
  auto const new_elements_to_old_elements =
      a.new_elements_to_old_elements.cbegin();
...
HPC_NOINLINE void
transfer_point_data(state const& s, adapt_state const& a, Range& data)
{
...
HPC_NOINLINE inline void
transfer_nodal_energy(input const& in, adapt_state const& a, state& s)
{
  auto const new_nodes_to_old_nodes = a.new_nodes_to_old_nodes.cbegin();
  for (auto const material : in.materials) {
    if (!in.enable_nodal_energy[material]) continue;
    hpc::device_vector<hpc::specific_energy<double>, node_index>& old_data =
        s.e_h[material];
    hpc::device_vector<hpc::specific_energy<double>, node_index> new_data(
        a.new_nodes.size());
...
template <class Range>
HPC_NOINLINE void
interpolate_nodal_data(adapt_state const& a, Range& data)
{
  interpolate_data(
      a.new_nodes,
...
adapt(input const& in, state& s)
{
  ...
  auto const num_new_elements =
      hpc::reduce(hpc::device_policy(), a.element_counts, element_index(0));
  auto const num_new_nodes =
      hpc::reduce(hpc::device_policy(), a.node_counts, node_index(0));
  hpc::offset_scan(
      hpc::device_policy(), a.element_counts, a.old_elements_to_new_elements);
  hpc::offset_scan(
  ...
  a.new_element_nodes_to_nodes.resize(
      num_new_elements * s.nodes_in_element.size());
  ...
  project(
      s.elements,
      a.old_elements_to_new_elements,
      a.new_elements_to_old_elements);
