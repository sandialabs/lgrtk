...
public:
  HPC_ALWAYS_INLINE HPC_HOST_DEVICE explicit constexpr material_set(material_index const material) noexcept : bits(std::uint64_t(1) << weaken(material)) {}
  ...
  HPC_ALWAYS_INLINE HPC_HOST_DEVICE constexpr static material_set all(material_index const size) noexcept {
    return material_set((std::uint64_t(1) << weaken(size)) - 1);
  }
