...
class state {
  ...
  device_vector<element_index, node_element_index> node_elements_to_elements{mempool};
  device_vector<node_in_element_index, node_element_index> node_elements_to_nodes_in_element{mempool};
  ...
  device_vector<symmetric3x3<double>, point_index> symm_grad_v{mempool}; // symmetrized gradient of velocity
  device_vector<double, point_index> p{mempool}; // pressure at elements (output only!)
  device_vector<vector3<double>, point_index> v_prime{mempool}; // fine-scale velocity
  device_vector<vector3<double>, point_index> q{mempool}; // element-center heat flux
  device_vector<double, point_node_index> W{mempool}; // work done, per element-node pair (contribution to a node's work by an element)
  ...
  device_vector<double, point_index> K{mempool}; // (tangent/effective) bulk modulus
  device_vector<double, point_index> G{mempool}; // (tangent/effective) shear modulus
  device_vector<double, point_index> c{mempool}; // sound speed / plane wave speed
  device_vector<vector3<double>, point_node_index> element_f{mempool}; // (internal) force per element-node pair (contribution to a node's force by an element)
  device_vector<vector3<double>, node_index> f{mempool}; // nodal (internal) forces
  device_vector<double, point_index> rho{mempool}; // element density
  device_vector<double, point_index> e{mempool}; // element specific internal energy
  device_vector<double, point_index> old_e{mempool}; // specific internal energy at previous time state
  device_vector<double, point_index> rho_e_dot{mempool}; // time derivative of internal energy density
  ...
  device_vector<double, point_index> nu_art{mempool}; // artificial kinematic viscosity scalar
  device_vector<double, point_index> element_dt{mempool}; // stable time step of each element
