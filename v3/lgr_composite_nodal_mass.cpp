  ...
  auto functor = [=] HPC_DEVICE (node_index const node) {
    ...
    for (auto const node_element : node_elements) {
      ...
      for (auto const node_in_element2 : nodes_in_element) {
        ...
        node_coords[weaken(node_in_element2)] = hpc::vector3<double>(nodes_to_x[node2].load());
      }
      ...
      for (auto const point_in_element : points_in_element) {
        ...
        point_densities(weaken(point_in_element)) = double(points_to_rho[point]);
      }
      ...
      m = m + coef[weaken(node_in_element)];
    }
    nodes_to_m[node] = m;
  };
