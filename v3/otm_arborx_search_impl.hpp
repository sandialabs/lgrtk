...
device_nearest_query_view make_nearest_node_queries(const device_point_view& points,
    const int num_nodes_to_find);

device_intersects_query_view make_intersect_sphere_queries(const device_sphere_view& point_spheres);

void inflate_sphere_query_radii(device_intersects_query_view queries, double factor);

template<typename query_view_type>
void do_search(const device_point_view& nodes, const query_view_type& queries, device_int_view &indices,
