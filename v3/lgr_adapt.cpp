...
HPC_NOINLINE inline void update_triangle_quality(state& s) noexcept {
  ...
  auto functor = [=] HPC_DEVICE (element_index const element) {
    ...
    for (auto const i : nodes_in_element) {
      grad_N[weaken(i)] = point_nodes_to_grad_N[point_nodes[i]].load();
    }
  ...
  auto functor = [=] HPC_DEVICE (node_index const node) {
    ...
    for (auto const node_element : nodes_to_node_elements[node]) {
      ...
      for (auto const node_in_element : nodes_in_element) {
        ...
        c.shell_elements_to_shell_nodes[shell_element][weaken(node_in_element)] = shell_node;
      }
      ...
      c.shell_elements_to_node_in_element[shell_element] = weaken(node_in_element);
    }
...
inline HPC_DEVICE void apply_triangle_swap(apply_cavity const c,
  ...
  for (auto const node_element : c.nodes_to_node_elements[node]) {
    ...
    node_in_element_index const plus1 = node_in_element_index((weaken(node_in_element) + 1) % 3);
    node_in_element_index const plus2 = node_in_element_index((weaken(node_in_element) + 2) % 3);
