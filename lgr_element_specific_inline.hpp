...
HPC_HOST_DEVICE inline hpc::area<double> triangle_area(hpc::array<hpc::position<double>, 3> const x) noexcept {
  return 0.5 * (cross((x[1] - x[0]), (x[2] - x[0]))(2));
}

HPC_HOST_DEVICE inline hpc::volume<double> tetrahedron_volume(hpc::array<hpc::position<double>, 4> const x) {
