...
interpolate_data(
    ...
    const hpc::device_array_vector<hpc::array<Index, 2, int>, Index>&
           interpolate_from_entities,
    Range& data)
{
  static_assert(
      std::is_same<Index, typename Range::size_type>::value,
  ...
  auto       functor                = [=] HPC_DEVICE(Index const i) {
    if (new_nodes_are_same[i]) {
      ...
      new_nodes_to_data[i] = 0.5 * (value_type(old_nodes_to_data[left]) +
    ...
    Range& data)
{
  ...
  auto       functor                = [=] HPC_DEVICE(Index const i) {
    if (new_nodes_are_same[i]) {
      ...
      Index const right = pair[1];
      new_nodes_to_data[i] =
          (1.0 / 3.0) * (value_type(old_nodes_to_data[left]) +
                         value_type(old_nodes_to_data[right]));
      new_nodes_to_data[left] =
          (2.0 / 3.0) * value_type(old_nodes_to_data[left]);
      new_nodes_to_data[right] =
          (2.0 / 3.0) * value_type(old_nodes_to_data[right]);
    }
  };
  hpc::for_each(hpc::device_policy(), new_entities, functor);
  data = std::move(new_data);
