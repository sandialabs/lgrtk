...
template <class Range, class UnaryFunction>
HPC_NOINLINE void
for_each(cuda_policy, Range&& r, UnaryFunction f)
{
...
HPC_ALWAYS_INLINE HPC_HOST_DEVICE void
copy(local_policy, FromRange const& from, ToRange& to) noexcept
{
...
template <class FromRange, class ToRange>
HPC_NOINLINE void
copy(serial_policy, FromRange const& from, ToRange& to)
{
  auto       first   = from.begin();
  auto const last    = from.end();
  auto       d_first = to.begin();
...
template <class FromRange, class ToRange>
HPC_NOINLINE void
copy(cuda_policy, FromRange const& from, ToRange& to)
{
...
HPC_ALWAYS_INLINE HPC_HOST_DEVICE void
move(local_policy, InputRange& input, OutputRange& output) noexcept
{
...
template <class InputRange, class OutputRange>
HPC_NOINLINE void
move(serial_policy, InputRange& input, OutputRange& output)
{
  auto       first   = input.begin();
  auto const last    = input.end();
  auto       d_first = output.begin();
...
template <class InputRange, class OutputRange>
HPC_NOINLINE void
move(cuda_policy policy, InputRange& input, OutputRange& output)
{
...
HPC_ALWAYS_INLINE HPC_HOST_DEVICE void
fill(local_policy, Range& r, T value) noexcept
{
  auto       first = r.begin();
...
template <class Range, class T>
HPC_NOINLINE void
fill(serial_policy, Range& r, T value)
{
  auto       first = r.begin();
  auto const last  = r.end();
  for (; first != last; ++first) { *first = value; }
...
template <class Range, class T>
HPC_NOINLINE void
fill(cuda_policy, Range& r, T value)
{
...
HPC_ALWAYS_INLINE HPC_HOST_DEVICE bool
any_of(local_policy policy, Range const& range, UnaryPredicate p) noexcept
{
...
template <class Range, class UnaryPredicate>
HPC_ALWAYS_INLINE HPC_HOST_DEVICE bool
all_of(local_policy policy, Range const& range, UnaryPredicate p) noexcept
{
...
HPC_ALWAYS_INLINE HPC_HOST_DEVICE bool
all_of(local_policy policy, Range const& range) noexcept
{
...
template <class Range, class UnaryPredicate>
bool
any_of(serial_policy policy, Range const& range, UnaryPredicate p)
{
  return transform_reduce(policy, range, false, logical_or(), p);
}
...
template <class Range, class UnaryPredicate>
bool
all_of(serial_policy policy, Range const& range, UnaryPredicate p)
{
  return transform_reduce(policy, range, true, logical_and(), p);
}
...
template <class Range>
bool
all_of(serial_policy policy, Range const& range)
{
  return all_of(policy, range, identity<bool>());
}
...
template <class Range>
bool
any_of(serial_policy policy, Range const& range)
{
