...
otm_compute_stress_average(state const& s)
{
  ...
  auto const          sigma_sum = hpc::transform_reduce(
      hpc::device_policy(),
      s.points,
      init,
      hpc::plus<hpc::stress<double>>(),
      functor);
...
otm_compute_deformation_gradient_average(state const& s)
{
  ...
  auto const                        F_sum = hpc::transform_reduce(
      ...
      s.points,
      init,
      hpc::plus<hpc::deformation_gradient<double>>(),
      functor);
...
void
otm_uniaxial_patch_test_ics(
    state&                    s,
    hpc::speed<double> const  top_velocity,
    hpc::adimensional<double> nu)
{
  auto const nodes_to_x = s.x.cbegin();
  auto const nodes_to_u = s.u.begin();
  auto const nodes_to_v = s.v.begin();
...
otm_j2_nu_zero_patch_test()
{
  ...
  auto const F33 =
      hpc::strain<double>(1.0 + top_displacement / hpc::length<double>(1.0));
  ...
  auto const error_sigma =
      hpc::norm(sigma_avg - sigma_gold) / hpc::norm(sigma_gold);
  ...
  std::cout << "STRESS GOLD:\n"
            << sigma_gold << "\nSTRESS AVERAGE:\n"
            << sigma_avg << '\n';
  std::cout << "STRESS ERROR: " << error_sigma
            << ", STRESS TOLERANCE: " << tol_sigma << '\n';
...
otm_j2_uniaxial_patch_test()
{
  ...
  auto const F11 =
      hpc::strain<double>(1.0 + trans_displacement / hpc::length<double>(1.0));
  auto const F22 =
      hpc::strain<double>(1.0 + trans_displacement / hpc::length<double>(1.0));
  auto const F33 =
      hpc::strain<double>(1.0 + top_displacement / hpc::length<double>(1.0));
  auto const F =
      hpc::deformation_gradient<double>(F11, 0, 0, 0, F22, 0, 0, 0, F33);
  auto const J         = hpc::determinant(F);
  ...
  auto       sigma_dev =
      hpc::transpose(hpc::inverse(F)) * Mdev * hpc::transpose(F) / J;
