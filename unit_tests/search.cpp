...
check_lgr_and_arborx_coords(
    ...
    const hpc::device_array_vector<hpc::position<double>, index_type>&
                                           lgr_coord,
...
void
check_arborx_spheres(
    const lgr::state&                  s,
    search::arborx::device_sphere_view search_spheres)
{
...
TEST_F(arborx_search, canDoNearestNodePointSearch)
{
  ...
  auto      queries = search::arborx::make_nearest_node_queries(
      search_points, num_nodes_per_point_to_find);
...
check_search_results(
    state& s,
    const hpc::device_vector<node_index, point_node_index>&
              points_to_supported_nodes_before_search,
    const int num_nodes_in_support_before_search)
{
  auto points_to_nodes_of_point = s.points_to_point_nodes.cbegin();
  auto old_points_to_supported_nodes =
      points_to_supported_nodes_before_search.cbegin();
  ...
  auto pt_node_check_func            = DEVICE_TEST(lgr::point_index point)
  {
    ...
    DEVICE_EXPECT_EQ(
        point_node_range.size(), num_nodes_in_support_before_search);
...
TEST_F(arborx_search, canDoNearestNodePointSearchThroughLGRInterface)
{
  ...
  hpc::device_vector<node_index, point_node_index>
      points_to_supported_nodes_before_search(s.point_nodes_to_nodes.size());
...
TEST_F(arborx_search, canDoIterativeSphereIntersectSearchOnExodusMesh)
{
  ...
  hpc::device_vector<node_index, point_node_index>
      points_to_supported_nodes_before_search(st.point_nodes_to_nodes.size());
...
void
check_inverse_connectivity(
    const lgr::state& s,
    const int         expected_num_pts_per_node)
{
...
TEST_F(
    distances_search,
    can_compute_nearest_and_farthest_node_to_node_from_search)
{
  state s;
  tetrahedron_single_point(s);
...
TEST_F(
    distances_search,
    can_compute_nearest_and_farthest_point_to_point_from_search)
{
  state s;
  two_tetrahedra_two_points(s);
