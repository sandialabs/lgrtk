...
Support::Support(Disc& disc_in, Subset* subset_in):disc(disc_in),subset(subset_in),
...
struct EntitySupport : public Support {
  ...
  Omega_h::Read<double> interpolate_nodal(
      int,
      Omega_h::Read<double>) override {
    Omega_h_fail("Only nodes and integration points can interpolate a nodal field\n");
  }
  bool on_points() override final { return false; }
};

int EntitySupport::count() {
  return subset->count();
}
...
struct NodeSupport : public EntitySupport {
  ...
  Omega_h::Read<double> interpolate_nodal(
      int ncomps,
      Omega_h::Read<double> node_values) override final;
};

Omega_h::Read<double> NodeSupport::interpolate_nodal(
    int ncomps,
    Omega_h::Read<double> node_values) {
  if (subset->mapping.is_identity) return node_values;
  return Omega_h::unmap(subset->mapping.things, node_values,
      ncomps);
}

template <class Elem>
Omega_h::Read<double> interpolate_nodal(
    MappedElemsToNodes elems_to_nodes,
    int ncomps,
    Omega_h::Read<double> node_values) {
...
struct PointSupport : public Support {
  ...
  {}
  int count() override final {
    return subset->count() * Elem::points;
  }
  Omega_h::Read<double> interpolate_nodal(
      int ncomps,
      Omega_h::Read<double> node_values) override final {
    return ::lgr::interpolate_nodal<Elem>(subset->ents_to_nodes(), ncomps, node_values);
  }
...
template struct PointSupport<Elem>;
