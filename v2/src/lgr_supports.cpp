...
Subset* const& SubsetOfSupportPtr::operator()(Support* s) {
  return s->subset;
}

Supports::Supports(Subsets& subsets_in):subsets(subsets_in) {}

Support* Supports::get_support(EntityType entity_type, bool on_points, ClassNames const& class_names) {
...
Support* Supports::get_support(Subset* subset, bool on_points) {
  ...
  if (it == by_subset.end() || (*it)->subset != subset) {
    Support* ptr;
    if (on_points) ptr = this->point_support_factory(disc, subset);
    else ptr = entity_support_factory(disc, subset);
...
template <class Elem>
void Supports::set_elem() { this->point_support_factory = lgr::point_support_factory<Elem>; }

#define LGR_EXPL_INST(Elem) \
template void Supports::set_elem<Elem>();
LGR_EXPL_INST_ELEMS
#undef LGR_EXPL_INST
