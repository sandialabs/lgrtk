...
Subset::Subset(
    Disc& disc_in, EntityType entity_type_in, ClassNames const& class_names_in)
    : disc(disc_in), entity_type(entity_type_in), class_names(class_names_in) {
...
void Subset::forget_disc() { mapping.things = decltype(mapping.things)(); }

void Subset::learn_disc() {
  if (mapping.is_identity) return;
  if (!mapping.things.exists()) {
    mapping.things = disc.ents_on_closure(class_names, entity_type);
  }
}
