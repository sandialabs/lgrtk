...
ClassNames const&
ClassNamesOfSubset::operator()(Subset const& ss) const {
  return ss.class_names;
}

SubsetPair const& SubsetPairOfSubsetBridge::operator()(SubsetBridge const& ssb) {
  return ssb.subsets;
}
...
Subset* Subsets::get_subset(EntityType type,
  ...
  if (it == by_class_names.end() ||
      it->class_names != class_names) {
...
void SubsetBridge::learn_disc(Subsets& all_subsets, TypeSubsets& type_subsets) {
  ...
  if (from == to) {
    ...
    Omega_h_fail("bridging from identity into "
    ...
    mapping.things = Omega_h::unmap(
        from->mapping.things, inv_to_map, 1);
    all_subsets.release_inverse(to_map);
  }
}

SubsetBridge* Subsets::get_bridge(
    Subset* from,
    Subset* to) {
