...
OMEGA_H_INLINE void sierra_J2_update(double const rho, double const E,
    double const nu, double const K, double const beta, double const Y,
    Tensor<3> const& F, Tensor<3>& Fp, double& eqps, Tensor<3>& sigma,
    double& c) {
...
struct SierraJ2 : public Model<Elem> {
  ...
  SierraJ2(Simulation& sim_in, Omega_h::InputMap& pl)
    ...
    this->plastic_def_grad =
        this->point_define("Fp", "plastic deformation gradient",
            square(this->sim.dim()), RemapType::PICK, pl, "I");
