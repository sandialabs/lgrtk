...
struct Indset : public Model<Elem> {
  ...
  void before_field_update() override final {
    ...
    auto const edge_qualities_w =
        Omega_h::Write<double>(sim.disc.mesh.nents(1));
    ...
    auto const indset_bytes =
        Omega_h::find_indset(&sim.disc.mesh, 1, edge_qualities, candidates);
