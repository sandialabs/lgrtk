...
OMEGA_H_INLINE void linear_elastic_update(double bulk_modulus,
    double shear_modulus, double density, Tensor<3> grad_u,
    Tensor<3>& stress, double& wave_speed) {
  auto const strain = (1.0 / 2.0) * (grad_u + transpose(grad_u));
  auto const I = identity_tensor<3>();
  auto const isotropic_strain = (trace(strain) / 3.) * I;
  ...
  auto const plane_wave_modulus = bulk_modulus + (4.0 / 3.0) * shear_modulus;
...
struct LinearElastic : public Model<Elem> {
  ...
  LinearElastic(Simulation& sim_in, Omega_h::InputMap& pl)
    ...
    this->effective_bulk_modulus = this->point_define("kappa_tilde",
        "effective bulk modulus", 1, RemapType::PICK, pl, "");
  }
  ...
  void at_material_model() override final {
    auto const points_to_kappa = this->points_get(this->bulk_modulus);
    auto const points_to_kappa_tilde = this->points_set(this->effective_bulk_modulus);
    auto const points_to_nu = this->points_get(this->shear_modulus);
    auto const points_to_rho = this->points_get(this->sim.density);
    auto const points_to_F = this->points_get(this->sim.def_grad);
    auto const points_to_F_init = this->points_get(this->sim.def_grad_init);
    auto const points_to_stress = this->points_set(this->sim.stress);
    auto const points_to_wave_speed = this->points_set(this->sim.wave_speed);
    auto functor = OMEGA_H_LAMBDA(int point) {
      auto const F = getfull<Elem>(points_to_F, point);
      auto const F_init = getfull<Elem>(points_to_F_init, point);
      auto const F_tot = F * F_init;
      auto const kappa = points_to_kappa[point];
      auto const nu = points_to_nu[point];
      auto const rho = points_to_rho[point];
      auto const I = identity_matrix<Elem::dim, Elem::dim>();
      auto const grad_u = resize<3>(F_tot - I);
      Tensor<3> sigma;
      double c;
