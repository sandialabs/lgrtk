...
Field::Field(std::string const& short_name_in, std::string const& long_name_in,
...
void Field::del() { storage = decltype(storage)(); }
...
void Field::setup_default_condition(Simulation& sim, double start_time) {
  ...
  conditions.insert(conditions.begin(),
      Condition(this, sim, default_value, support, at_time(start_time)));
}
