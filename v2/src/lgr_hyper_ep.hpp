...
OMEGA_H_INLINE double absmax(Tensor<3> a) {
  double m = std::abs(a(0, 0));
  for (int i = 1; i < 3; i++) m = Omega_h::max2(m, std::abs(a(i, i)));
  return m;
}

OMEGA_H_INLINE bool is_deviatoric(Tensor<3> const a) {
  return std::abs(trace(a)) <= 1.0e-12;
}

OMEGA_H_INLINE Tensor<3> deviatoric_part(Tensor<3> const a) {
  auto tr = trace(a) / 3.0;
  ...
  if (!is_deviatoric(dev)) {
    printf(
        ...
        dev(0, 0), dev(1, 1), dev(2, 2), dev(0, 1), dev(1, 2), dev(0, 2),
        trace(dev));
    OMEGA_H_CHECK(false);
  }
  return dev;
...
void read_and_validate_damage_params(
    Omega_h::InputMap& params, Properties& props);
void read_and_validate_eos_params(Omega_h::InputMap& params, Properties& props);
 ...
 */
OMEGA_H_INLINE void hard_constant(double& Y, double& dY, double const /* ep */,
    double const /* epdot */, double const /* temp */, double const Y0) {
  Y = Y0;
  dY = 0.0;
}
 ...
 * plastic strain.
 */
OMEGA_H_INLINE void hard_linear_isotropic(double& Y, double& dY,
...
OMEGA_H_INLINE void hard_power_law(double& Y, double& dY, double const ep,
    double const /* epdot */, double const /* temp */, double const A,
    double const B, double const n) {
 ...
 */
OMEGA_H_INLINE void hard_johnson_cook(double& Y, double& dY, double const ep,
    double const epdot, double const temp, double const A, double const B,
    double const n, double const T0, double const TM, double const m,
 ...
 *
 */
OMEGA_H_INLINE void hard_zerilli_armstrong(double& Y, double& dY,
    double const ep, double const epdot, double const temp, double const A,
    double const B, double const n, double const C, double const alpha_0,
    double const alpha_1, double const D, double const beta_0,
    double const beta_1) {
  double const f = A + B * std::pow(ep, n);
  double const df =
      (std::abs(ep) < 1.0e-14) ? 0.0 : n * B * std::pow(ep, n - 1.0);

  double const alpha = alpha_0 - alpha_1 * std::log(epdot);
  double const g = C * std::sqrt(ep) * std::exp(-alpha * temp);
  double const dg = (std::abs(ep) < 1.0e-14)
                        ? 0.0
...
OMEGA_H_INLINE void hard(double& Y, double& dY, double const ep,
    double const epdot, double const temp, Properties const props) {
  if (props.hardening == Hardening::NONE) {
    ...
    hard_zerilli_armstrong(Y, dY, ep, epdot, temp, props.p0, props.p1, props.p2,
        props.p3, props.p4, props.p5, props.p6, props.p7, props.p8);
  } else if (props.hardening == Hardening::JOHNSON_COOK) {
    hard_johnson_cook(Y, dY, ep, epdot, temp, props.p0, props.p1, props.p2,
        props.p3, props.p4, props.p5, props.p6, props.p7);
  }
}

OMEGA_H_INLINE void scalar_damage_johnson_cook(double& dp, double const pres,
    Tensor<3> const s, double const /* ep */, double const epdot,
    double const temp, double const dtime, double const D1, double const D2,
    double const D3, double const D4, double const D5, double const D6,
    double const D7, double const eps_f_min) {
...
OMEGA_H_INLINE void scalar_damage(double& dp, double const pres,
    Tensor<3> const s, double const ep, double const epdot, double const temp,
    double const dtime, Properties const props) {
  if (props.damage == Damage::JOHNSON_COOK) {
    scalar_damage_johnson_cook(dp, pres, s, ep, epdot, temp, dtime, props.D1,
        props.D2, props.D3, props.D4, props.D5, props.D6, props.D7, props.D8);
  }
}

// Returns the Mandel stress
OMEGA_H_INLINE void mandel(double& pres, Tensor<3>& s, Tensor<3> const F,
    Tensor<3> const Fp, Properties const props) {
...
OMEGA_H_INLINE void elastic(double& pres, Tensor<3>& s, double& wave_speed,
    double const J, Tensor<3> const F, Tensor<3> const Fp, double const rho,
    Properties const props) {
  OMEGA_H_CHECK(props.elastic == Elastic::MANDEL);
  ...
  if (props.eos == EOS::MIE_GRUNEISEN) {
    ...
    mie_gruneisen_update(
        props.rho0, props.gamma0, props.cs, props.s1, rho, props.e0, pres, c);
    K = rho * c * c;
  } else {
    // wave speed
...
OMEGA_H_INLINE bool at_yield(Tensor<3> const s, double const ep,
    double const epdot, double const temp, Properties const props) {
 ...
 *
 */
OMEGA_H_INLINE void radial_return(Tensor<3>& s, double const /*J*/,
    Tensor<3> const /*F*/, Tensor<3>& Fp, double& ep, double& epdot,
    double const temp, double const /*dtime*/, Properties const props) {
  OMEGA_H_CHECK(is_deviatoric(s));
...
OMEGA_H_INLINE void update_damage(double& pres, Tensor<3>& s, double& dp,
    int& localized, double const ep, double const epdot, double const temp,
    double const dtime, Properties const props) {
...
OMEGA_H_INLINE_BIG void update(Properties const props, double const rho,
    Tensor<3> const F, double const dtime, double const temp, Tensor<3>& T,
    double& wave_speed, Tensor<3>& Fp, double& ep, double& epdot,
    double& /*dp*/, double& /*localized*/) {
#ifdef LGR_HAVE_SIERRA_J2
  if (props.hardening == Hardening::SIERRA_J2) {
    sierra_J2_update(rho, props.E, props.Nu, props.p1, props.p2, props.p0, F,
        Fp, ep, T, wave_speed);
    if (props.eos == EOS::MIE_GRUNEISEN) {
      double pres = 0.0;
      double c = 0.0;
