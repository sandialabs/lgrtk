...
class array_vector_reference
{
 public:
  ...
  using iterator_type    = ::hpc::impl::inner_iterator<
      ::hpc::
          pointer_iterator<array_value_type, decltype(O() * array_size_type())>,
      L,
      O,
      array_size_type>;
...
class array_vector_reference<T const, L, O>
{
 public:
  ...
  using array_size_type  = typename ::hpc::array_traits<T>::size_type;
  using iterator_type    = ::hpc::impl::inner_iterator<
      ::hpc::pointer_iterator<
          array_value_type const,
          decltype(O() * array_size_type())>,
      L,
      O,
      array_size_type>;
 ...
 public:
  HPC_ALWAYS_INLINE HPC_HOST_DEVICE explicit array_vector_reference(
      iterator_type iterator_in) noexcept
      : m_iterator(iterator_in)
  {
  }
  // movable
...
class array_vector_iterator
{
 ...
 public:
  ...
  HPC_ALWAYS_INLINE HPC_HOST_DEVICE explicit constexpr array_vector_iterator(
      iterator iterator_in) noexcept
      : m_iterator(iterator_in)
  {
  }
...
class array_vector
{
 ...
 private:
  using matrix_allocator_type = typename std::allocator_traits<
      Allocator>::template rebind_alloc<array_value_type>;
  using matrix_type = ::hpc::matrix<
      array_value_type,
      L,
      matrix_allocator_type,
      ExecutionPolicy,
      Index,
      array_size_type>;
 ...
 public:
  ...
  using const_reference =
      ::hpc::impl::array_vector_reference<value_type const, L, Index>;
  using pointer        = T*;
...
using host_array_vector = array_vector<
    T,
    ::hpc::host_layout,
    ::hpc::host_allocator<T>,
    ::hpc::host_policy,
    Index>;
template <class T, class Index = std::ptrdiff_t>
using device_array_vector = array_vector<
    T,
    ::hpc::device_layout,
    ::hpc::device_allocator<T>,
    ::hpc::device_policy,
    Index>;
template <class T, class Index = std::ptrdiff_t>
using pinned_array_vector = array_vector<
    T,
    ::hpc::device_layout,
    ::hpc::pinned_allocator<T>,
    ::hpc::host_policy,
    Index>;
...
void
copy(
    pinned_array_vector<T, Index> const& from,
    device_array_vector<T, Index>&       to)
{
  ...
  using array_value_type =
      typename pinned_array_vector<T, Index>::array_value_type;
#ifndef NDEBUG
  auto err =
#endif
...
void
copy(
    device_array_vector<T, Index> const& from,
    pinned_array_vector<T, Index>&       to)
{
  assert(from.size() == to.size());
  auto const num_arrays = from.size();
  auto const array_size = from.array_size();
