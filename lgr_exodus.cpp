...
void read_exodus_file(std::string const& filepath, input const& in, state& s) {
  ...
  s.nodes.resize(node_index(init_params.num_nodes));
  s.elements.resize(element_index(init_params.num_elem));
  s.material.resize(s.elements.size());
  host_vector<int> host_conn(s.elements.size() * s.nodes_in_element.size());
  int offset = 0;
  for (int i = 0; i < init_params.num_elem_blk; ++i) {
    ...
    assert(nnodes_per_entry == s.nodes_in_element.size());
    ...
    exodus_error_code = ex_get_conn(exodus_file, EX_ELEM_BLOCK, block_ids[i],
        host_conn.data() + offset * s.nodes_in_element.size(), edge_conn.data(),
        face_conn.data());
    assert(exodus_error_code == 0);
    auto material_begin = s.material.cbegin() + element_index(offset);
  ...
  for (auto const element : s.elements) {
    ...
    for (auto const node_in_element : s.nodes_in_element) {
      int const host_index = int(element * s.nodes_in_element.size() + node_in_element);
      auto const element_node = [node_in_element];
  ...
  pinned_conn.clear();
  host_vector<double> host_coords[3];
  ...
  for (auto const node : s.nodes) {
    pinned_coords[node] = vector3<double>(
        host_coords[0][int(node)],
        host_coords[1][int(node)],
        host_coords[2][int(node)]);
  }
