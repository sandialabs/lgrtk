...
template <
    typename NodeRelRangeType,
    typename NodeRelToRelRangeType,
    typename NodeRelToNodesOfRelRangeType>
void
otm_sort_node_relations(
    lgr::state&                   s,
    NodeRelRangeType&             nodes_to_node_relations,
    ...
    NodeRelToNodesOfRelRangeType& node_relations_to_nodes_of_relation)
{
  ...
  auto node_rel_to_nodes_of_node_rel =
      node_relations_to_nodes_of_relation.begin();
  auto sort_functor = [=] HPC_DEVICE(node_index const node) {
    auto const                                  this_node_rel = node_rels[node];
    typename NodeRelRangeType::value_type const except_last(
        this_node_rel.begin(), this_node_rel.end() - 1);
    for (auto const nr : except_last) {
      typename NodeRelRangeType::value_type const remaining(
          nr + 1, this_node_rel.end());
      ...
      hpc::swap(
          node_rel_to_nodes_of_node_rel[nr],
          node_rel_to_nodes_of_node_rel[min_node_rel]);
    }
#ifndef NDEBUG
    for (auto i(*(this_node_rel.begin())); i < (*(this_node_rel.end())) - 1;
         ++i) {
